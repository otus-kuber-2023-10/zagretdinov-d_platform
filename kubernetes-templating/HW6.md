...
Общая логика задачи следующая:
 ...
 | Рекомендуем не заглядывать в сниппеты в ссылках и попробовать самостоятельно разобраться с jsonnet В качестве подсказки  
 | можно использовать и готовый services.jsonnet , который должен выглядеть примерно следующим образом: services.jsonnet
